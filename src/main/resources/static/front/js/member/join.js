window.addEventListener("DOMContentLoaded", function() {
    ...
    const authCodeEl = document.getElementById("auth-coed");
    ...
    sendButton.addEventListener("click", function() {
         ...
         * 코드 전송 완료 후 후속 처리
         *
         * 1. 전송버튼의 문구를 인증코드 재 전송으로 변경
         * 2. 타이머에 남은 시간을 출력
         */

        emailAuth.sendCode(email, updateTimer, function() {
             // 1. 전송버튼의 문구를 인증코드 재 전송으로 변경, 이메일을 변경하지 못하게 처리
             // 인증 코드 입력 가능하게 처리
    ...
    */
    function updateTimer(seconds) {
        ...
        } else { // 타이머가 0이되면 다시 이메일 변경 가능하게 처리, 인증 코드 입력 불가 처리
